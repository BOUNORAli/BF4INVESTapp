...
public class BandeCommandeService {
    ...
    public BandeCommande create(BandeCommande bc) {
        ...
        auditService.logCreate("BandeCommande", saved.getId(),
                "BC " + saved.getNumeroBC() + " créée" + clientsInfo + " - Total: " + saved.getTotalVenteTTC()
                        + " MAD");
    ...
    public BandeCommande update(String id, BandeCommande bc) {
        return bcRepository.findById(id)
                .map(existing -> {
                    ...
                    // Infos livraison (nouveaux champs) - permettre la suppression (null ou chaîne
                    // vide)
                    existing.setLieuLivraison(bc.getLieuLivraison() != null && !bc.getLieuLivraison().trim().isEmpty()
                            ? bc.getLieuLivraison()
                            : null);
                    existing.setConditionLivraison(
                            bc.getConditionLivraison() != null && !bc.getConditionLivraison().trim().isEmpty()
                                    ? bc.getConditionLivraison()
                                    : null);
                    existing.setResponsableLivraison(
                            bc.getResponsableLivraison() != null && !bc.getResponsableLivraison().trim().isEmpty()
                                    ? bc.getResponsableLivraison()
                                    : null);
                    ...
                    // IMPORTANT: Préserver les totaux Excel si présents (pour aligner avec les
                    // factures)
     ...
     * Exemple : ABC01XYZ01/25 (Client ABC, mois 01, Fournisseur XYZ, ordre 01,
     * année 2025)
     * 
     ...
     * @throws IllegalArgumentException si la date BC, le client ou le fournisseur
     *                                  sont manquants
     */
    private String generateBCNumber(BandeCommande bc) {
        ...
        // IMPORTANT : On utilise la date BC (pas la date de création) pour déterminer
        // le mois/année
        ...
        long count = bcRepository.findAll().stream()
                .filter(existingBc -> {
                    ...
                    if (existingBc.getDateBC() == null)
                        return false;
                    if (existingBc.getDateBC().getMonthValue() != month)
                        return false;
                    if (existingBc.getDateBC().getYear() != year)
                        return false;
        ...
        // 6. Générer le numéro d'ordre : toujours utiliser 2 chiffres (01, 02, 03,
        // etc.)
        // Le comptage est basé uniquement sur le mois/année de la date BC, pas sur la
        // date de création.
    ...
    private String normalizeReference(String reference) {
        ...
        // Supprimer les espaces et caractères spéciaux, garder seulement lettres et
        // chiffres
    ...
    private void calculateTotals(BandeCommande bc) {
        // DEBUG: Tracer l'état avant calcul
        if (bc.getFournisseursAchat() != null && bc.getFournisseursAchat().size() > 1) {
            log.info("calculateTotals: BC {} a {} fournisseurs AVANT calcul",
                    bc.getNumeroBC(), bc.getFournisseursAchat().size());
        }

        // Migration automatique si nécessaire
        migrateToMultiFournisseur(bc);

        // DEBUG: Tracer l'état après migration
        if (bc.getFournisseursAchat() != null && bc.getFournisseursAchat().size() > 1) {
            log.info("calculateTotals: BC {} a {} fournisseurs APRÈS migration",
                    bc.getNumeroBC(), bc.getFournisseursAchat().size());
        }
     ...
     * Si fournisseurId existe mais fournisseursAchat est vide, créer un
     * FournisseurAchat
     */
    private void migrateToMultiFournisseur(BandeCommande bc) {
        if (bc.getFournisseursAchat() == null || bc.getFournisseursAchat().isEmpty()) {
            // DEBUG
            log.debug(
                    "migrateToMultiFournisseur: BC {} a fournisseursAchat vide/null, tentative migration depuis legacy",
                    bc.getNumeroBC());
            if (bc.getFournisseurId() != null) {
     ...
     * PRIORITÉ : Utilise les totaux bruts Excel si disponibles, sinon calcule
     * depuis les lignes
     */
    private void calculateTotalsMultiClient(BandeCommande bc) {
        ...
        if (totalAchatHT > 0) {
            bc.setMargePourcentage(
                    NumberUtils.roundTo2Decimals(((totalVenteHTGlobal - totalAchatHT) / totalAchatHT) * 100));
        } else {
     ...
     * - Surplus = Quantité achetée - Quantité vendue totale (somme sur tous les
     * clients)
     ...
     * 
     * Exemple : Achat 100, Vente Client1=40, Vente Client2=30 → Ajoute 30 au stock
     * (surplus)
     * 
    ...
    public void updateStockFromBC(BandeCommande bc) {
        ...
        // Calculer les quantités vendues totales par produit (somme sur tous les
        // clients)
        ...
        for (LigneAchat ligne : bc.getLignesAchat()) {
            ...
            // Si pas de clients ou pas de ventes pour ce produit, quantiteVendueTotale sera
            // 0.0,
